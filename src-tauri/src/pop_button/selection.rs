// Ownership rules shared by the native event worker and its tests. No Win32,
// clipboard, configuration or window calls belong in this module.
...
impl PendingGesture {
    ...
    pub fn capture_clipboard(
        ...
        // Preserve the original one-update fallback. A later clipboard change
        // invalidates this candidate; it does not re-arm this old gesture for
        // unrelated clipboard content, even within the 600ms arrival window.
...
impl Gesture {
    ...
    pub fn interrupted_by(self, window: isize, at_ms: u32) -> bool {
        // WinEvent delivery can lag behind a later mouse event. Ignore events
        // from before this gesture, including across the tick clock wrap.
...
impl Candidates {
    ...
    pub fn complete_uia(
        ...
        // Consume before the sink applies length/language filters. A filtered
        // UIA answer must never allow unrelated clipboard text to bypass them.
...
mod tests {
    ...
    fn cancel_and_new_gesture_reject_queued_answers_without_text_deduplication() {
        ...
        for g in [old, new] {
            state.begin(g);
            state.cancel(999); // A queued cancel cannot cancel a newer gesture.
    ...
    fn old_hide_and_duplicate_engage_do_not_consume_a_new_button() {
        ...
        assert!(display.take(1).is_none()); // delayed Hide or Engage
    ...
    fn foreground_changes_invalidate_even_if_the_user_returns_to_the_window() {
        ...
        assert!(!g.interrupted_by(11, 900)); // delayed event before this gesture

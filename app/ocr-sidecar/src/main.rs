...
fn main() {
    // args_os 而不是 args：后者遇到不合法的 UTF-16（用户名里有落单的代理对，NTFS 不校验）
    // 直接 panic，进程什么都没干就死了。路径全程 OsString（主程序侧 R-7 同理）。
    ...
    let ocr: WechatOcrFn = unsafe {
        ...
        if module.is_null() {
            die(&format!("LoadLibraryW failed: {}", dll.to_string_lossy()));
        }
        let proc = GetProcAddress(module, c"wechat_ocr".as_ptr().cast());
        if proc.is_null() {
    ...
    // imgfn 只能是窄字符，所以图片路径这一个必须是合法 UTF-8。
    // 它是主程序自己在缓存目录下生成的（`ocr_region_<pid>_<n>.png`），正常一定能转。
    let img = match image.to_str().map(CString::new) {
        Some(Ok(v)) => v,
        Some(Err(_)) => die("image path contains a NUL byte"),
        None => die("image path is not valid UTF-8"),
    };
    let ok = unsafe {
        ocr(
            wide(engine).as_ptr(),
    ...
    unsafe {
        let module = LoadLibraryW(wide(dll).as_ptr());
        let proc = GetProcAddress(module, c"stop_ocr".as_ptr().cast());
        if !proc.is_null() {
